...
While they are quite immature at the moment and not very wildly used, this will change in the future and WepKit will certainly become a valuable option at some point.
...
In server applications, we suggest using a virtual display such as Xvfb to fulfil that requirement.

In operation the application parses the command-line given, which triggers loading of supplied URLs (local or remote) in dedicated browser widgets.
When the load complete event fires, one of several extraction routines is run and results written back to disk.
Implemented extraction routines are 
\begin{description}
...
Clients access it via CGI scripts executed by a Web server while the back-end uses python wrapper scripts.
...
It also serves the actual HTML data, whereas any embedded objects are served by the separate proxy server.
...
The ``CoordRect'' JAMF component generates masks the same size of the Web page screen-shot based on the node coordinates.
