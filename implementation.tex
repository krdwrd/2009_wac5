...
Whereas the open source variant of Google's browser, \textit{Chromium}, promises superior execution speed by coupling WebKit with it's own V8 JavaScript engine, it suffers from the same problem as WebKit itself of not being stable enough yet to serve as reliable platform -
...
\item[merge\label{merge}] for merging different annotations on the same Web page into one in a simple voting scheme,
